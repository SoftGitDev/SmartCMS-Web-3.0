...
import { KeywordType } from '../types/ApiConfigData';
import { extractKeywordsFromTemplate } from '../../../services/storage/common';
...
const APIConfigFrm: React.FC<APIConfigFrmProps> = ({ show, handleClose, servicesDtlData, isEdit, getServiceData }) => {
    ...
    const getAPITypeCombo = useCallback(async () => {
        try {
            const payload = {};
            const config = {};
            const result = await apiRequest("POST", urls.getAPITypeCombo, payload, config)
    ...
    const addServiceData = async (val: any,) => {
        ...
        try {
            ...
            const config = {};
            const result = await apiRequest("POST", urls.addApiService, payload, config)
    ...
    const editServiceData = async (val: typeof initialValues) => {
        try {
            ...
            const config = {};
            const result = await apiRequest("PUT", urls.editApiService, payload, config)
            if (result.status === '200' && result.success) {
                handleClose();
                getServiceData();
            } else {
