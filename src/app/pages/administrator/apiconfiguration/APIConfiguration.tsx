...
import APIConfigFrm from './APIConfigFrm';
import LoaderUI from '../../../components/loader/Loader';
import APIConfigTest from './APIConfigTest';
import { ApiConfigDataProps, servicelistProps } from '../../../types/administrator';
import PageHeaeder from '../../../components/common/PageHeaeder';
import ImportMdl from '../../../components/common/ImportMdl';
import APIConfigCard from '../../../content/card/APIConfigCard';

// const PageHeaeder = lazy(() => import("../../../components/common/PageHeaeder").then(({ default: PageHeaeder }) => ({ default: PageHeaeder })));
// const ConfirmationMdl = lazy(() => import("../../../content/modal/ConfirmationMdl").then(({ default: ConfirmationMdl }) => ({ default: ConfirmationMdl })));
// const APIConfigCard = lazy(() => import("../../../components/ui/Card/APIConfigCard").then(({ default: APIConfigCard }) => ({ default: APIConfigCard })));
// const APIConfigFrm = lazy(() => import("./APIConfigFrm").then(({ default: APIConfigFrm }) => ({ default: APIConfigFrm })));
...
const APIConfiguration = () => {
    ...
    const handleEdit = useCallback(async (tranCode: number, index: number) => {
        try {
            ...
            await apiRequest("POST", urls.getApiServiceDetail, payload, config).then((result) => {
    ...
    const getServiceData = useCallback(async () => {
        try {
            ...
            await apiRequest("POST", urls.getApiServiceList, payload).then((result) => {
                if (result.success && result.status === "200") {
                    const response = result.response || [];
    ...
    const handleTestDetails = useCallback(async (tranCode: number, index: number) => {
        try {
            ...
            await apiRequest("POST", urls.getServiceTestDetail, payload, config).then((result) => {
                if (result.Status === "200" && result.Success) {
                    const response = result.Response;
                    // Open modal with service details
    ...
    const exportApiConfig = useCallback(async (tranCode: number, index: number) => {
        try {
            ...
            await apiRequest("POST", urls.exportApiConfig, payload, config).then((result) => {
                if (result.Success) {
                    const response = result.Response;
